(function($) {
  ...
  // Sammy.FormFactory is a utility function for creating a dictionary
  // of form elements for simple use with Handlebars templates.
  //
  // The fields parameter is a dictionary mapping keypath to field type:
  //    { key_one : "text",
  //      key_two : ["select", ["opt1", "opt2"]] }
  //
  // if Sammy.SafeString is available then that is used to wrap the results
  Sammy.FormFactory = function(name, object, fields) {
    var result = {}
    var form = new Sammy.FormBuilder(name, object)
    for (var key in fields) {
        var field = fields[key]
        if ($.isArray(field)) {
            var field_type = field[0];
            var field_args = [key].concat(field.slice(1));
            result[key] = form[field_type].apply(form, field_args);
        } else {
            result[key] = form[field](key);
        }
        
        if (Sammy.SafeString)
            result[key] = new Sammy.SafeString(result[key])
    }
    return result;
  }
